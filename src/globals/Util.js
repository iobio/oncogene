...
class Util {
  ...
  formatCurrentDateTime(delim) {
    ...
    today = mm + theDelim + dd + theDelim + yyyy + theDelim + theTime;
    return today;
  }
  ...
  */
  ...
  //     emailObject.filename = 'gene.iobio.screencapture.' + self.formatCurrentDateTime('.') + '.html';
  //    } else {
  //     emailObject.filename = '';
  //    }
  //
  //   client.on('open', function(stream){
  //     client.createStream(emailObject);
  ...
  removeUrl(paramName) {
    ...
    window.location.search.split('&').forEach(function(param){
    if (param.indexOf(paramName) > 0 && param != '') {
        param = param.split('?').length == 1 ? param : param.split('?')[1];
        var fields = param.split('=');
        params[fields[0]] = fields[1];
      }
      ...
      for(var key in value) {
        ...
        if (tokens.length == 2) {
          ...
          if (variant.vepConsequence && Object.keys(variant.vepConsequence).length > 0) {
            for(var consequence in variant.vepConsequence) {
