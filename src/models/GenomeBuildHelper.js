  ...
  */
  getBuildsInHeaders(bamHeaderMap, vcfHeaderMap) {
    ...
    for (relationship in bamHeaderMap) {
      header = bamHeaderMap[relationship];
      buildInfo = me.getBuildFromBamHeader(header);
      me.parseBuildInfo(buildInfo, relationship, 'bam', theBuilds);
    }
    for (relationship in vcfHeaderMap) {
      header = vcfHeaderMap[relationship];
      buildInfo = me.getBuildFromVcfHeader(header);
  ...
  getProperSpeciesAndBuild(buildInfo) {
    ...
    if (buildInfo != null) {
      ...
      if (buildInfo.species) {
        for (var speciesName in me.speciesNameToSpecies) {
          if (!matchedSpecies) {
