...
<template>
  <v-card flat class="my-1 variant-card">
        ...
        <v-expansion-panel-content :value="openState">
          <v-card flat id="card-viz">
            ...
            <div style="width:100%" id="viz-div">
              ...
              <v-chip v-if="((!this.sampleModel.isTumor) && this.somaticOnlyMode)"
              ...
              <div class="chart-label"
                   v-show="showDepthViz && sampleModel.rnaSeqCoverage && sampleModel.rnaSeqCoverage.length > 1">
                rna-seq
              </div>
...
export default {
  ...
  methods: {
    ...
    showVariantCircle: function (variant, lock) {
      if (this.showVariantViz && this.openState[0] === 0) {
    ...
    hideVariantCircle: function (lock) {
      const self = this;
      if (self.showVariantViz && this.openState[0] === 0) {
        let container = this.getVariantSVG();
    ...
    getVariantSVG: function () {
      if (this.d3.select(this.$el).select('#card-viz')) {
  ...
  mounted: function () {
    ...
    if ((!this.sampleModel.isTumor) && this.somaticOnlyMode) {
      this.openState[0] = 1;
    }
  },
  created: function () {
